...
## Obsidian connector (designed, not yet built)

The second connector. Status: **design settled; implement after the prompt
rewrite + eval harness land** — notes are the content type the current prompt
handles worst, and card-level detection will drive real usage on day one.

### Why Obsidian next, and why it isn't "folders again"

Folder import was cut because a raw filesystem walk doesn't understand what it
walks. An Obsidian vault is self-describing, which removes every objection:
